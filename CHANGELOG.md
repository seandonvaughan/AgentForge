...
- Added `agentforge cycle preview --objective "<text>"` (spec 2026-05-30 §13 m3): rehearses the epic decomposition — planner exploration + the full deterministic validation pipeline — and prints the children table, dependency waves with per-wave cost, file-overlap forced orderings, the budget band `[0.7, 1.0]×spendable`, warnings, and the planner's own LLM cost, without running a cycle (no git, no worktrees, no execution). Supports `--json`; honors `--budget-usd` (defaults to `budget.perCycleUsd`); artifacts persist to `.agentforge/previews/objective-<ts>/` (never `.agentforge/cycles/`, which `cycle list` would misread as a cycle); exits 1 with band/cycle/missing-predecessor detail when the decomposition is invalid. `DecomposeError` now carries the LLM cost incurred before a failure so even a failed rehearsal reports its price.
