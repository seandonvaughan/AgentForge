...
describe('GET /api/v5/codex/readiness', () => {
  ...
  it('serves repeated readiness requests from a short route cache', async () => {
    const projectRoot = makeProjectRoot();
    const app = Fastify({ logger: false });
    ...
    expect(readinessReportBuilder).toHaveBeenCalledTimes(2);
    expect(first.json<{ meta: { cached: boolean; cacheTtlMs: number } }>().meta).toMatchObject({
      cached: false,
      cacheTtlMs: 60_000,
    });
    expect(second.json<{ meta: { cached: boolean; cacheTtlMs: number } }>().meta).toMatchObject({
      cached: true,
      cacheTtlMs: 60_000,
    });
    expect(doctor.json<{ meta: { cached: boolean; cacheTtlMs: number } }>().meta.cached).toBe(false);
