...
export async function cyclesRoutes(
  ...
  app.get('/api/v5/cycles/:id', async (req, reply) => {
    ...
    // Aggregate cost / tests / agent activity from per-phase files. The
    // execute phase writes phases/<name>.json on completion, so we get real
    // numbers for finished phases without waiting for terminal cycle.json.
