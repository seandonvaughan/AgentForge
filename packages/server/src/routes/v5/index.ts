...
/** Exported for unit-test access. Forwards canary lifecycle envelopes as SSE events. */
export function bridgeCanaryLifecycleToGlobalStream(
  ...
  const action = envelope.topic === 'self-modification.canary.staged'
    ? 'staged'
    : envelope.topic === 'self-modification.canary.promoted'
      ? 'promoted'
      : 'rolled back';
  ...
  globalStream.emit({
    ...
    category: 'quality',
    message: `Self-mod canary ${action}`,
    payload: {
