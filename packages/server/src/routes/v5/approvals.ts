...
export interface ApprovalsOptions {
  /** Project root containing the .agentforge/ directory. Defaults to monorepo root. */
  projectRoot?: string;
}

// ---------------------------------------------------------------------------
// Internal row type (SQLite snake_case columns → camelCase ApprovalItem)
// ---------------------------------------------------------------------------
...
export async function approvalsRoutes(
  ...
  // Close the connection when the Fastify app shuts down
  app.addHook('onClose', async () => {
    db.close();
  });

  // GET /api/v5/approvals — list approval queue with optional status filter
  ...
  // PATCH /api/v5/approvals/:id/rollback — mark an approved item as rolled back
