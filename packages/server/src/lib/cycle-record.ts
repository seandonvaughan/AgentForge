/**
 * Shared cycle-record reader for the AgentForge server package.
 *
 * Canonical source of truth for reading a CycleRecord from disk.  Any server-
 * side code that needs to parse a cycle directory should import from here so
 * there is a single implementation to maintain.
 *
 * NOTE: The SvelteKit dashboard package (packages/dashboard/src/routes/flywheel/
 * +page.server.ts) keeps a parallel copy of this logic to support SSR without a
 * runtime dependency on this package.  If you change the on-disk format handling
 * here, mirror the same change there.  A future step will consolidate both into
 * @agentforge/shared once the dashboard package gains that dependency.
 */
