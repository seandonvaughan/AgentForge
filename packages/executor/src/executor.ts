...
import { buildPlan, isSelfModificationProposal } from './planner.js';
import type {
  ExecutionResult,
  ExecutorOptions,
...
export class ProposalExecutor {
  ...
  async execute(proposal: AgentProposal): Promise<ExecutionResult> {
    ...
    const stagesToRun = plan.stages.filter((stage) => this.shouldExecuteStage(stage, proposal));
    let totalMs = 0;
    let totalCostUsd = 0;
    const diffs: string[] = [];
  ...
  private shouldExecuteStage(stage: ExecutionStage, proposal: AgentProposal): boolean {
    if (stage === 'complete' || stage === 'failed') return false;
    if (stage === 'canary' && !this.opts.canaryEnabledForSelfModification && isSelfModificationProposal(proposal)) {
      return false;
    }
    return true;
  }
