...
const DEFAULT_OPUS_MODEL_ID = 'gpt-5.5';
const DEFAULT_HAIKU_MODEL_ID = 'gpt-5.4-mini';
const DEFAULT_CODEX_PROFILES = {
...
function readCodexProfileConfig(root) {
    const empty = { tiers: {} };
...
function parseCodexProfileConfig(content) {
    const config = { tiers: {} };
    ...
    for (const rawLine of content.split('\n')) {
        ...
        if (indent === 4) {
            ...
            currentTier = null;
            if (!inTiers && value)
                assignProfileValue(config, key, value);
            continue;
        }
        if (!inTiers)
            continue;
        if (indent === 6) {
            ...
            if (currentTier && !config.tiers[currentTier])
                config.tiers[currentTier] = {};
            continue;
        }
...
function assignProfileValue(target, key, value) {
    if (key === 'model' || key === 'modelId')
        target.modelId = value;
    if (key === 'effort')
        target.effort = value;
}
...
function isCapabilityTier(value) {
