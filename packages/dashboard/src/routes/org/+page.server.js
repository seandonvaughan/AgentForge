 ...
 *     - report2
 */
function parseDelegationYaml(content) {
    const result = {};
    let current = null;
 ...
 */
export function _buildOrgGraph(projectRoot) {
    ...
    function addEdge(from, to) {
        if (EXCLUDED_AGENTS.has(from) || EXCLUDED_AGENTS.has(to))
            return;
        const key = `${from}\0${to}`;
        if (edgeSet.has(key))
            return;
    ...
    for (const [id, content] of agentContents) {
        ...
        for (const to of asStringArray(collab.can_delegate_to))
            addEdge(id, to);
    }
...
export const load = () => _buildOrgGraph(findProjectRoot());
