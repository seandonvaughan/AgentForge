...
function writeCycle(id, overrides = {}) {
...
function writeSprintFlat(filename, items) {
    const dir = join(tmpRoot, '.agentforge/sprints');
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, filename), JSON.stringify({ version: filename.replace('.json', ''), items }));
}
...
function writeSession(filename, data) {
...
describe('computeMetrics — memoryStats (memory loop health card)', () => {
    function writeMemoryEntry(type, source, createdAt) {
...
describe('computeMetrics — debug counters', () => {
    it('counts cycles, sprints, agents, items, and sessions correctly', () => {
        mkdirs(join(tmpRoot, '.agentforge/cycles'), join(tmpRoot, '.agentforge/sprints'), join(tmpRoot, '.agentforge/agents'));
        writeCycle('c1');
...
describe('computeMetrics — events.jsonl cycle format', () => {
    function writeCycleFromEvents(id, opts = {}) {
        const { sprintVersion = '1.0.0', stage = 'completed', startedAt = '2026-01-01T00:00:00.000Z', completedAt = '2026-01-01T01:00:00.000Z', passed = 100, failed = 0, prNumber = 1, totalCostUsd = 50, } = opts;
        const dir = join(tmpRoot, '.agentforge/cycles', id);
        mkdirSync(dir, { recursive: true });
        const events = [
...
describe('computeMetrics — cycles-archived/ reading', () => {
    function writeArchivedCycle(id, overrides = {}) {
...
describe('computeMetrics — sprints/archive/ reading', () => {
    function writeArchiveSprint(filename, items) {
...
describe('computeMetrics — plan.json in cycle directories', () => {
    function writePlanJson(cycleId, items) {
