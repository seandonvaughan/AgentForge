...
async function loadRenderMarkdown() {
    ...
    if (!moduleScript)
        throw new Error('MarkdownRenderer module script was not found');
    const transformedImports = moduleScript.replace(/from 'marked';/, `from '${pathToFileURL(markedPath).href}';`);
