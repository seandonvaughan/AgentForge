...
import { createApprovalsStore } from '../lib/stores/approvals.js';
let lastMockES = null;
class MockEventSource {
    ...
    constructor(url) {
        ...
        lastMockES = {
            url,
            get onopen() { return MockEventSource.instances.at(-1)?.onopen ?? null; },
            get onmessage() { return MockEventSource.instances.at(-1)?.onmessage ?? null; },
            get onerror() { return MockEventSource.instances.at(-1)?.onerror ?? null; },
            close: () => this.close(),
            push: (data) => {
...
function makeStore(preseededPending = []) {
