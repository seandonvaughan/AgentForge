...
// (opus|sonnet|haiku) or a lone effort hint — which is all the agent-YAML
// fallback provides — resolves to the PRIMARY (Claude) family's model id,
// because forcing the cycle onto codex was exactly the bug. Genuine codex runs
// still render their real gpt-* id because the runtime records the concrete
// model string on each run.
//
// The export names retain the historical `codex*` prefix to avoid churning the
// four call sites; the behavior is provider-neutral.
...
function tierFromModelId(raw: string): CapabilityTier {
  ...
  // bare gpt-* without a codex/mini marker → treat as the top tier
