...
const initial = { full: '…', short: 'v…', major: 'v…', loaded: false };
export const versionStore = writable(initial);
/** Convenience derived store for components that just need the short label. */
export const versionShort = derived(versionStore, ($v) => $v.short);
export const versionFull = derived(versionStore, ($v) => $v.full);
export const versionMajor = derived(versionStore, ($v) => $v.major);
let loaded = false;
/**
