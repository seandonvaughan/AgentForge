...
describe('epic-planner agent registration', () => {
  ...
  it('claude agent md (when generated) has fable frontmatter', () => {
    ...
    // tier check (project lesson #7: assert behaviour, not generated files).
    const mdPath = join(ROOT, '.claude/agents/epic-planner.md');
    if (!existsSync(mdPath)) return;
    const md = readFileSync(mdPath, 'utf8');
    expect(md).toMatch(/name:\s*epic-planner/);
    expect(md).toMatch(/model:\s*fable/);
  });

  it('team.yaml lists epic-planner under strategic + fable routing', () => {
    const t = load(readFileSync(join(ROOT, '.agentforge/team.yaml'), 'utf8')) as any;
    expect(t.agents.strategic).toContain('epic-planner');
    expect(t.model_routing.fable).toContain('epic-planner');
  });

  it('routing-index has a fable-tier epic-planner agent', () => {
