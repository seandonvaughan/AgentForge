...
function readRuntimeRoutingSummary(cycleDir: string): RuntimeRoutingSummary | null {
  ...
  const runs = Array.isArray(execute.itemResults)
    ? execute.itemResults
    : (Array.isArray(execute.agentRuns) ? execute.agentRuns : []);
  if (runs.length === 0) return null;

  const decisions: RuntimeRoutingDecision[] = runs
    .map((run) => {
    ...
    .filter((decision): decision is RuntimeRoutingDecision => decision !== null);
