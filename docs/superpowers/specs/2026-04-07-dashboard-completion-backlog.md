...
✅ DONE: /approvals dashboard page is fully wired to the cycle budget-gate endpoints (v6.7.4+). `packages/dashboard/src/lib/stores/approvals.ts` creates `approvalsStore` which polls `GET /api/v5/cycle-sessions` every 10 seconds (with SSE as the hot-path), filters sessions where `hasApprovalPending === true`, fetches each pending cycle's full approval payload from `GET /api/v5/cycles/:id/approval?workspaceId=...`, and exposes `pending[]`, `active`, `loading`, and `error` to the UI. The global `ApprovalModal` component (`packages/dashboard/src/lib/components/ApprovalModal.svelte`) is mounted in `+layout.svelte` above all pages and subscribes to the store — it renders within-budget items as a scrollable checkbox list with individual cost totals and a live "selected cost / budget" meter that turns amber when over budget. Quick-select buttons ("Within budget only", "Select all", "Clear") are available. The "Approve N items" button POSTs `{approvedItemIds, rejectedItemIds, decidedBy}` to `POST /api/v5/cycles/:id/approval`; "Approve all" sends `{approveAll: true}`; "Reject all" sends the inverse. The server broadcasts an `approval.decision` SSE event on success so all connected dashboard tabs dismiss the cycle immediately without waiting for the next poll. On `approval.pending` SSE events the modal auto-opens so operators don't miss the gate. The `/approvals` page (`+page.svelte`) subscribes to the store and renders a "Cycle Budget Approvals" card for each pending cycle with a "Review N items" button that opens the modal. 16 unit tests in `packages/dashboard/src/__tests__/approvals-store.test.ts` cover the SSE event paths, refresh() data cascade, workspaceId forwarding, auto-open behavior, and error handling — all pass.
